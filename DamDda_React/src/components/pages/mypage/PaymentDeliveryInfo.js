...
export default function PaymentDeliveryInfo({ project }) {
  return (
    <div className="payment-delivery-info">
      <h4>배송 정보</h4>
      <p>수령인: {project.delivery.name}</p>
      <p>휴대폰: {project.delivery.phoneNumber}</p>
      <p>주소: {project.delivery.address}</p>
      <p>배송 요청 사항: {project.delivery.message}</p>

      <h4>결제 정보</h4>
      <p>결제 방법: {project.payment.paymentMethod}</p>
      <p>총 상품 금액: {parseInt(project.supportingPackage.paymentPrice).toLocaleString()}원</p>
      <p>결제 상태: {project.payment.paymentStatus}</p>
    </div>
  );
}
