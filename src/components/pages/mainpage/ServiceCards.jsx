...
export const ServiceCards = () => {
  return (
    <Box sx={{ width: '70%', padding: '40px 0', backgroundColor: '#f8f8f8', display: 'flex',justifyContent: 'center' ,margin: '0 auto'  }}>
      <Grid container justifyContent="center" spacing={5} sx={{ maxWidth: '1400px'}}>
        {services.map((service, index) => (
          <Grid item xs={12} sm={6} md={4} key={index}>
            ...
            >
              <Box sx={{ flexGrow: 1 }}>
                <Typography variant="subtitle2" sx={{ fontWeight: '600',fontSize:'18px' }}>
                  {service.title}
                </Typography>
                <Typography variant="body1" sx={{ marginTop: 1, color:'white',fontSize:'12px' }}>
                  {service.description}
                </Typography>
              </Box>
